...
class WordService {
    ...
    async getWordbook(ctx: Context) {
      ctx.body = { wordbook: this.wordbook }
    }
  
    async generateSentences(ctx: Context) {
      ...
      const prompt =
        '利用英文单词（words）生成一个英文段落，要求这个段落不超过100个英文单词且必须全英文，并包含上述英文单词，同时是一个有逻辑的句子'
...
export function addRouter (router: any) {
    // 添加一个单词
